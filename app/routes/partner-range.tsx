...
<section className="section light"><div className="shell"><div className="range-grid">{ranges.map((r,i)=>{const colours=partnerQuartz.filter(c=>c.tier===r.name);return <article className="range-card" key={r.name}><div className={`range-swatch swatch-${i+1}`}><span>{r.pos}</span></div><div className="range-card-copy"><p className="eyebrow dark">Partner Range</p><h2>{r.name}</h2><h3>{r.tag}</h3><p>{r.copy}</p><div className="colour-list">{colours.map(c=><a className="colour-chip colour-chip-link" href={`/materials/quartz/partner-range/${slugify(c.name)}`} key={c.name}>{c.name}<span>→</span></a>)}</div><p><strong>{colours.length} curated colours</strong></p><a className="text-link" href="/worktop-matcher">Not sure? Let StoneMatch recommend one <span>→</span></a></div></article>})}</div></div></section>
