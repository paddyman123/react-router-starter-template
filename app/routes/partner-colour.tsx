...
export default function PartnerColour({params}:any){
 ...
 useEffect(()=>{if(!colour)return;const list=JSON.parse(localStorage.getItem("stonematch-shortlist")||"[]");setSaved(list.includes(colour.name));},[colour?.name]);
 ...
 const toggle=()=>{const list:string[]=JSON.parse(localStorage.getItem("stonematch-shortlist")||"[]");const next=list.includes(colour.name)?list.filter(x=>x!==colour.name):[...list,colour.name];localStorage.setItem("stonematch-shortlist",JSON.stringify(next));setSaved(next.includes(colour.name));window.dispatchEvent(new Event("stonematch-shortlist"));};
 return <Page>
  <section className="page-hero colour-hero"><div className="shell narrow"><p className="eyebrow">Partner Range · {colour.tier}</p><h1>{colour.name}</h1><p className="lead">A curated StoneMatch quartz selected for its combination of design, practicality and value.</p><div className="colour-actions"><a className="button gold" href={`/?colour=${encodeURIComponent(colour.name)}#enquiry`}>Get a quote in this colour →</a><button className="shortlist-button" onClick={toggle}>{saved?"♥ Saved to shortlist":"♡ Add to my shortlist"}</button></div></div></section>
