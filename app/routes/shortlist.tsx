...
export default function Shortlist(){
 ...
 useEffect(()=>{const sync=()=>{try{setNames(JSON.parse(localStorage.getItem("stonematch-shortlist")||"[]"))}catch{setNames([])}};sync();window.addEventListener("stonematch-shortlist",sync);return()=>window.removeEventListener("stonematch-shortlist",sync);},[]);
 ...
 const enquiryHref=`/?material=Quartz&shortlist=${encodeURIComponent(names.join(", "))}&source=shortlist#enquire`;
 return <Page>
  ...
  <section className="section light"><div className="shell">{colours.length===0?<div className="empty-shortlist"><p className="eyebrow dark">Nothing saved yet</p><h2>Start with the <em>Partner Range.</em></h2><p>Tap “Add to my shortlist” on any colour you like. Your favourites will stay saved while you browse.</p><a className="button gold" href="/materials/quartz/partner-range">Explore Partner Range →</a></div>:<><div className="shortlist-summary"><strong>{colours.length} {colours.length===1?"colour":"colours"} saved</strong><span>Your favourites are stored on this device until you are ready to send them to us.</span></div><div className="shortlist-grid">{colours.map((colour:any)=><article className="shortlist-card" key={colour.name}><a href={`/materials/quartz/partner-range/${slugify(colour.name)}`}><div className="mini-stone"><span>{colour.name}</span></div></a><div className="shortlist-copy"><p className="eyebrow dark">{colour.tier}</p><h3><a href={`/materials/quartz/partner-range/${slugify(colour.name)}`}>{colour.name}</a></h3><p>{colour.tags.slice(0,3).join(" · ")}</p><button onClick={()=>remove(colour.name)}>Remove</button></div></article>)}</div><div className="shortlist-send"><div><p className="eyebrow dark">Ready for us to help?</p><h2>Send your shortlist to <em>StoneMatch.</em></h2><p>We'll review the colours you've selected alongside your kitchen, budget and priorities, then look for a strong-value installed route. Your saved colours will be added to the enquiry automatically.</p></div><a className="button gold" href={enquiryHref}>Find the best value for my shortlist →</a></div></>}</div></section>
 </Page>;
}
